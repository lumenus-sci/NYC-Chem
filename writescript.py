...
for seq, mon, day, hr in zip(seqs, mons, days, hrs):
    with open(f'wrf_ghg_{f"{seq:02}" if isinstance(seq, int) else seq}.sh','a') as fl:
        ...
        if seq == 'real':
            ...
            fl.write('\n')
            fl.write('ln -sf $HOME/mozbc/*2024*.inp .\n')
            fl.write('ln -sf $HOME/mozbc/CO2CAMS*.inp .\n')
            fl.write('ln -sf $HOME/mozbc/mozbc_* .\n')
            fl.write('ln -sf $HOME/work/chem-files/CH4/CH4_CAMS/cams73_latest_ch4_conc_surface_satellite_inst_2023_00??.nc .\n')
            fl.write('./mozbc_ch4 < CH4_2024yearly08.inp >& CH4_d01.log\n')
            fl.write('./mozbc_ch4 < CH4_2024yearly08_d02.inp >& CH4_d02.log\n')
            fl.write('\n')
            fl.write('ln -sf $HOME/work/chem-files/CO2_CAMS/cams73*2023* .\n')
            fl.write('\n')
            fl.write('./mozbc_co2 < CO2CAMS_2024yearly08.inp >& CO2_d01.log\n')
            fl.write('./mozbc_co2 < CO2CAMS_2024yearly08_d02.inp >& CO2_d02.log\n')
            fl.write('\n')
        else:
